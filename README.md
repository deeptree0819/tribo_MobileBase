...
> **depth 를 보려면 `enable_depth:=false` 로 띄우면 안 됩니다.** 그 상태에서도 토픽 이름은 광고되지만 데이터가 흐르지 않아 화면이 검게만 보입니다.
>
> depth 는 컬러 영상이 아니라 **16비트 거리값(`16UC1`, 단위 mm)** 이라 그대로는 표시할 수 없습니다(`[16UC1] is not a color format` 오류). 뷰어가 인코딩을 감지해 자동으로 컬러맵을 입힙니다.
>
> - **파랑 = 가까움, 빨강 = 멀음** (표시 범위 0.3~4.0 m, 스크립트 상단 `DEPTH_MIN_M`/`DEPTH_MAX_M`)
> - **검정 = 측정 실패** — 유리·거울·검은 물체·너무 가까운 면에서 나옵니다. 0 을 그대로 칠하면 "코앞에 물체가 있음"으로 보이므로 일부러 검게 남깁니다
>
> 매 프레임 최소/최대로 정규화하지 않고 **고정 범위**를 쓰는 이유는, 장면이 바뀔 때마다 같은 거리가 다른 색으로 보이면 실습에 쓸 수 없기 때문입니다.
