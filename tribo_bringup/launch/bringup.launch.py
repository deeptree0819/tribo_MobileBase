...
def generate_launch_description():
    ...
    _odom_common_params = {
        ...
        # 2026-08-19 구조 변경(물리 track 0.71 -> 0.74) + 100mm 메카넘 교체.
        #   아래는 시드값일 뿐 실측이 아니다. 82mm 슬립비 1.096/0.71 = 1.544 를
        #   그대로 써서 0.74 x 1.544 = 1.143 -> 1.14.
        #   ⚠️ 100mm 롤러는 접지 특성이 다르고, 슬립비는 track 보다 휠베이스에
        #      더 민감하다(구조 변경으로 휠베이스도 바뀌었을 수 있음). 제자리
        #      N바퀴 회전 + 출발선 복귀 오차로 반드시 재측정할 것:
        #        new = 1.14 x (odom 누적 yaw / 물리 실제 yaw)
        #   ⚠️ wheel_radius 확정(직진 캘리브) 이후에 측정해야 한다. R 이 틀리면
        #      회전 odom 이 그만큼 통째로 스케일돼 이 값이 오염된다.
        # 2026-08-19 실측 확정 (기체 7b6a, 100mm 메카넘). 시드 1.14 -> 1.1845.
        #   제자리 3바퀴법: odom 1080.6도 vs 물리 1040도(2바퀴+320) -> x1.0390.
        #   슬립비(유효/물리) = 1.1845/0.74 = 1.601.
        #   참고로 76c02a + 82mm 메카넘은 1.096/0.71 = 1.544 였다. 메카넘 슬립비가
        #   1.5~1.6 대라는 게 두 기체에서 확인된 셈이다.
        # ⚠️ 물리 track 이 기체마다 다르다(76c02a 0.71 / 7b6a 0.74). 7b6a 구조 변경
        #    때문이다. 이 값은 SHARED 인데 실제로는 per-unit 이어야 맞다. 다른 기체를
        #    쓸 때는 반드시 이 방법으로 재측정할 것 (per-unit override 경로 없음).
